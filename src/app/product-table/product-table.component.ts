...
export class ProductTableComponent implements OnInit {
  ...
  getAttributes(pageNo) {
    axios
      .get(
        'https://dadyin-product-server-7b6gj.ondigitalocean.app/api/attributes/?page=' +
          pageNo
      )
      .then((response) => {
        for (let index = 0; index < response.data.results.length; index++) {
          ...
          if (this.usedAttributes.includes(element.description)) {
            const att = this.attributeList.find(
              (d) => d.description === element.description
            );
            if (!att) {
  ...
  calculateTotalConversioncost() {
    ...
    for (let index = 0; index < this.addedConversion.length; index++) {
      ...
      const cost =
        !element.cost || element.cost === '' ? 0.0 : parseFloat(element.cost);
  ...
  calcluateTotalCost() {
    this.totalCost = 0.0;
    const total =
      parseFloat(this.totalCostAddOn.toString()) +
      parseFloat(this.totalConversionCost.toString());
  ...
  onProductPercentUsedChangeHandler(event: any, i: number) {
    ...
    if (this.isProductAttribute(selectedProduct, 'PercentUsed')) {
      ...
      this.processSchema.products[i].productAttributeValues.push(
        this.addPercentUsed(productPercetUsed)
      );
    }

    const productWaste = this.getAttributeValueByName(
      selectedProduct,
      'PercentWaste'
    );
    const productdensity = this.getAttributeValueByName(
      selectedProduct,
      'Density'
    );
    const productAvgDesity = this.calculateAvgDenstiy(
      productPercetUsed,
      productWaste,
      productdensity
    );
    ...
    if (this.isProductAttribute(selectedProduct, 'AverageDensity')) {
      ...
      this.processSchema.products[i].productAttributeValues.push(
        this.addNewAverageDensity(productAvgDesity)
      );
    }

    const productCost = this.getAttributeValueByName(selectedProduct, 'Cost');
    const costAddon = this.calculateAddOn(
      productPercetUsed,
      productWaste,
      productCost
    );
    if (this.isProductAttribute(selectedProduct, 'CostAddOn')) {
      ...
      this.processSchema.products[i].productAttributeValues.push(
        this.addNewCostAddOn(costAddon)
      );
    }
  ...
  onProductWasteChangeHandler(event: any, i: any) {
    ...
    if (this.isProductAttribute(selectedProduct, 'PercentWaste')) {
      ...
      this.processSchema.products[i].productAttributeValues.push(
        this.addPercentWaste(productWaste)
      );
    }

    const productPercetUsed = this.getAttributeValueByName(
      selectedProduct,
      'PercentUsed'
    );
    const productdensity = this.getAttributeValueByName(
  ...
  setProductAttribute(i, attributeName, value) {
    ...
    for (
      let index = 0;
      index < this.processSchema.products[i].productAttributeValues.length;
      index++
    ) {
      const element =
        this.processSchema.products[i].productAttributeValues[index];
      if (element.attribute.description === attributeName) {
        this.processSchema.products[i].productAttributeValues[
          index
        ].attributeValue = value;
      }
    }
  ...
  addPercentUsed(value) {
    const percenUsedAttribute =
      this.getSingleAttributeBydescription('PercentUsed');
    return {
  ...
  calculateAvgDenstiy(percetageUsed, waste, density) {
    ...
    return (
      waste
        ? (Math.abs(xpercetageUsed - xwaste) * xdensity) / 100
        : (xpercetageUsed * xdensity) / 100
    ).toFixed(6);
  }
  ...
  selectConverionChangeHandler(event: any, i: number) {
    ...
    const selectedConversion = this.ConversionName.find(
      (d) => d.id.toString() === conversionId
    );
  ...
  xgetProcessProducts() {
    ...
    for (let index = 0; index < this.selectedProducts.length; index++) {
      ...
      if (this.xcostAddon != '')
        arr.push(this.getCostAdOnAttribute(this.xcostAddon));
  ...
  updateSchemaProduct() {
    return this.createdProcessResponseArray.map((d) => ({
      product: d.id,
      processProductAttributeValues: d.productAttributeValues,
    }));
  }
